...
class MetaLoader(object):
    ...
    def __init__(self, loaders, accum_steps=1, distributed=False):
        ...
        for idx, (n, l) in enumerate(loaders.items()):
            ...
            self.sampling_probs.append(prob)
    ...
    def __iter__(self):
        ...
        while True:
            if self.step % self.accum_steps == 0:
                # task_idx = self.step%len(self.sampling_pools)
                task_idx = np.random.choice(range(len(self.sampling_pools)), 1, p=self.sampling_probs)[0]
                # print("task==============={}".format(task_idx))
                task = self.sampling_pools[task_idx]
            ...
            try:
                ...
                batch = next(iter_)
                # e_t= time.time()
                # print("load time:{}".format(s_t-e_t))
            except StopIteration:
                self.epoch = self.epoch + 1
                if "distcc4m" in task or "laion2b" in task or "tuchong" in task or "distcc12m" in task or "coyo700m" in task :
                    pass
                else:
