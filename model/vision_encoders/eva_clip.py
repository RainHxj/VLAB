    ...
    def forward(self, x, attn_mask: Optional[torch.Tensor] = None):
        ...
        if attn_mask is not None:
            if attn_mask.dtype == torch.bool:
                ...
                x_len = attn.shape[1]
                attn_mask = attn_mask[:,:x_len,:x_len]
...
class TextTransformer(nn.Module):
    ...
    def forward_features(self, text: torch.Tensor):
        ...
        x_len = x.shape[1]
        x = x + self.positional_embedding[:x_len]
...
class EVA_CLIP(nn.Module):
    def __init__(self,
                 ...
                 adapt: str = None,
                 batch_size: int = 1,
                 start_adapt_layer: int = -1,
                 adapt_order: str = 'adapt_last',
                 adapt_cls_dim: int = 64,
                 adapt_patch_dim: int = 16):
        super().__init__()
        ...
        print(f"img_size={vision_cfg.image_size}, patch_size={vision_cfg.patch_size}, num_classes={embed_dim}, use_mean_pooling={False}, init_values={vision_cfg.layer_scale_init_value}")
        print(f"embed_dim={vision_cfg.width},depth={vision_cfg.layers},num_heads={vision_heads},mlp_ratio={vision_cfg.mlp_ratio}")
        print(f"drop_path_rate={vision_cfg.drop_path_rate},adapt={adapt},batch_size={batch_size},start_adapt_layer={start_adapt_layer},adapt_order={adapt_order}")
        print(f"adapt_cls_dim={adapt_cls_dim},adapt_patch_dim={adapt_patch_dim},grad_checkpointing={grad_checkpointing}")
