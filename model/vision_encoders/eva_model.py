...
from .eva_clip import EVA_CLIP, convert_weights_to_fp16
    ...
    for cf in config_files:
        with open(cf, 'r') as f:
            ...
            if all(a in model_cfg
                   for a in ('embed_dim', 'vision_cfg', 'text_cfg')):
...
def load_checkpoint(model,
    ...
    w = checkpoint['visual.pos_embed'].squeeze(0).cuda()
    src_cls = w[0:1]
    src_oth = w[1:]
    grid = resolution // 14
    dim = w.size(-1)
    ...
    checkpoint['visual.pos_embed'] = tgt.unsqueeze(0).cpu()
    state_dict = checkpoint
...
def create_model(model_name: str,
    ...
    model_cfg['adapt'] = adapt
    model_cfg['batch_size'] = batch_size
    model_cfg['start_adapt_layer'] = start_adapt_layer
    model_cfg['adapt_order'] = adapt_order
    model_cfg['adapt_cls_dim'] = adapt_cls_dim
    model_cfg['adapt_patch_dim'] = adapt_patch_dim
    model_cfg['grad_checkpointing'] = grad_checkpointing
    model_cfg['vision_cfg']['image_size'] = image_size

    model = EVA_CLIP(**model_cfg)

    load_checkpoint(model, pretrained, resolution=image_size)
...
def build_eva_model_and_transforms(model_name: str,
                                   pretrained: str = '',
                                   precision: str = 'fp32',
                                   device: torch.device = torch.device('cpu'),
                                   force_quick_gelu: bool = False,
                                   image_mean: Optional[Tuple[float,
                                                              ...]] = None,
                                   image_std: Optional[Tuple[float,
                                                             ...]] = None,
                                   adapt: str = None,
                                   batch_size: int = 1,
                                   start_adapt_layer: int = -1,
