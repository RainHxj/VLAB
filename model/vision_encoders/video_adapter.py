...
class VideoAdapter(nn.Module):
    ...
    def forward(self, x: Tensor, bs, mask=None):
        ...
        # adapt more frames.
        if pos_emd.size(0) != cls_down.size(1):
            # c_frame, dim
            pos_emd = F.interpolate(pos_emd.unsqueeze(0).permute(0, 2, 1), cls_down.size(1), mode='linear').permute(0, 2, 1).squeeze(0)
...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x_tuple: tuple, attn=None):
        ...
        #     x3 = x1 + self.img_adapter(self.text_adapter(x2))
        # elif self.adapt_order == 'adapt_first':
        #     x2 = self.video_adapter(x1, bs, mask)
        #     x2 = self.img_adapter(self.text_adapter(x2))
        #     x3 = self.mlp(self.ln_2(x1))
        #     x3 = x2 + x3
        if self.training and self.use_checkpoint:
        ...
        if self.adapt_order == 'adapt_last':
            ...
            x1 = torch.utils.checkpoint.checkpoint(self.video_adapter, x1, bs,
                                                   mask)
            x1 = self.img_adapter(self.text_adapter(x1))
        x3 = x1 + x2
        return (x3, video_frame, bs, idx + 1, mask)
