...
def preprocess_gts(idsfile,tokenizer):
    ann_data = pickle.load(open(idsfile,"rb"))
    output = defaultdict(list)
    for anno in ann_data:
        for tt in anno["text"]:
            caption = tokenizer.encode(tt)
            output[anno['filename']].append(caption)
    return output


class Scorer(object):
    def __init__(self, idsfile, tokenizer):
        super(Scorer, self).__init__()
        self.scorers = []
        #self.weights = cfg.SCORER.WEIGHTS
        #self.gts = pickle.load(open(cfg.SCORER.GT_PATH, 'rb'), encoding='bytes')

        document_frequency, ref_len = precompute_df_reflen_for_cider(idsfile, tokenizer)

        self.gts = preprocess_gts(idsfile,tokenizer)
...
def precompute_df_reflen_for_cider(idsfile, tokenizer):
    gts = []

    crefs= []
    document_frequency = defaultdict(int)
    if idsfile.startswith("hdfs"):
        raise NotImplementedError
    else:
        ann_data = pickle.load(open(idsfile,"rb"))
        # ids = [ann["filename"] for ann in ann_data]
    # anns = json.load(open(annfile))['annotations']
    id2anns = defaultdict(list)
    for a in ann_data:
        id2anns[a['filename']].append(tokenizer.encode(a['text']))  
