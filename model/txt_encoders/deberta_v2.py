...
from transformers.models.deberta_v2.configuration_deberta_v2 import DebertaV2Config as DebertaV2Config

from .bert import BertCrossAttention, BertCrossOutput
import ipdb
...
class DebertaV2Layer(nn.Module):
    def __init__(self, config):
        ...
        self.cross_attn = BertCrossAttention(config=config)
        self.cross_output = BertCrossOutput(config=config)
    ...
    def forward(
        ...
        if cross_hidden_states is not None:
            cross_attention_output = self.cross_attn(attention_output, cross_hidden_states)
            attention_output = self.cross_output(cross_attention_output, attention_output)
...
class DebertaV2Model(DebertaV2PreTrainedModel):
    ...
    def forward(
        ...
        task_prompt: Optional[torch.Tensor] = None,
        ...
        if not self.has_cross_attn:
            ...
            cross_hidden_states = []
            if video_feat is not None:
                cross_hidden_states.append(video_feat) 
            if len(cross_hidden_states) > 0:
                cross_hidden_states = torch.cat(cross_hidden_states,dim=1)
            else:
                cross_hidden_states = None
            attention_mask = torch.ones(*input_feat.shape[:2]).long().cuda() 
