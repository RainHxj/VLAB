...
__all__ = [
    ...
    'load_from_hdfs', 'hsave_pkl', 'hdelete', 'hmove'
]
...
def hload_mmcv(filepath, flag='color', channel_order='rgb', backend=None):
    ...
    with hopen(filepath) as f:
        image_str = f.read()
        img = mmcv.imfrombytes(image_str, flag=flag, channel_order=channel_order, backend=backend)
...
def hload_torch(filepath, map_location='cpu', **kwargs):
...
def hsave_torch(obj, filepath, **kwargs):
    """ save model to hdfs """
    if filepath.startswith("hdfs://"):
        with hopen(filepath, "wb") as writer:
...
# def load_from_hdfs(filename, map_location='cpu'):
#     """load checkpoint through HDFS scheme path. In distributed
...
#     """
#     checkpoint = hload_torch(filepath=filename, map_location=map_location)
#     return checkpoint
...
#     f = hload_pkl("hdfs://haruna/home/byte_labcv_default/liyinan/msrvtt/annotation/test_Retri_1k_anno.pkl")
#     print(f)
