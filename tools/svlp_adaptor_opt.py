...
def convert_model(model,out_name=None):
    ...
    for key,value in model_weight.items():
        if key.startswith('vbackbone.vision_model.embeddings.class_embedding'):
            ...
            elif "ext_layer_norm2.bias" in key:
                new_key = "visual.transformer.resblocks.{}.ext_ln_2.bias".format(num_layers)
                models_clip[new_key] = value    
            elif "ext_layer_norm2.weight" in key:
                new_key = "visual.transformer.resblocks.{}.ext_ln_2.weight".format(num_layers)
                models_clip[new_key] = value               
            elif "ext_mlp.fc1.weight" in key:
                new_key = "visual.transformer.resblocks.{}.ext_mlp.c_fc.weight".format(num_layers)
                models_clip[new_key] = value
            elif 'ext_mlp.fc1.bias' in key:
                new_key = "visual.transformer.resblocks.{}.ext_mlp.c_fc.bias".format(num_layers)
                models_clip[new_key] = value
            elif 'ext_mlp.fc2.weight' in key:
                new_key = "visual.transformer.resblocks.{}.ext_mlp.c_proj.weight".format(num_layers)
                models_clip[new_key] = value
            elif 'ext_mlp.fc2.bias' in key:
                new_key = "visual.transformer.resblocks.{}.ext_mlp.c_proj.bias".format(num_layers)
                models_clip[new_key] = value
    ...
    torch.save(clip_out, "{}.pth".format("vlp_clip_large_coyo_adaptor_coca"))
